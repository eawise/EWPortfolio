...
define( 'CHILD_THEME_NAME', 'Coastal' );
...
//Load language files
function coastal_lang_setup() {
    load_theme_textdomain( 'coastal', get_stylesheet_directory() . '/languages' );
...
add_action( 'after_setup_theme', 'coastal_lang_setup' );
...
//Configure ACF PRO
    ...
    $coastal_acf_pro = get_theme_mod('coastal_acf_pro', 0);
    if ($coastal_acf_pro != 1) {
...
//* Register images 
add_image_size( 'blog-featured', 1680, 9999, FALSE );
...
//Fix smooth scroll conflict with WooCommerce
...
function coastal_register_header() {
    add_theme_support( 'genesis-custom-header', array( 'width' => '3000', 'height' => '1700' ) );
}
...
function coastal_custom_footer() {
    ...
    if ( $coastal_display_copyright == '' ) {
        echo '<p>' , __('Copyright ', 'coastal' ) , date("Y ") , bloginfo("name");
        }
    else { 
        echo '<p>' , get_theme_mod( 'copyright-footer' ) ; 
        }
    if ( $coastal_stnsvn_credit == '') {
    echo  '<span id="stnsvn-credit"> | ' , __('Site design handcrafted by', 'coastal') , '<a href="http://stnsvn.com" target="_blank"> Station Seven</span>';
    } else {
    echo '</p>'; //Close footer p tag
    }
}
...
function coastal_post_meta_filter($post_meta) {
    ...
    $post_meta = '[post_categories before="'.__('Filed Under', 'coastal').': "] [post_tags before="'.__('Tagged with', 'coastal').': "]';
    if ( is_singular( 'post' )) {
        ...
        if ( $coastal_display_categories != '' ) {
            $blog_meta_categories = '[post_categories before="'.__('Filed Under: ', 'coastal').'"]'; 
        }
        
        if ( $coastal_display_tags != '' ) {
            $blog_meta_tags = ' [post_tags before="'.__('Tagged with: ', 'coastal').'"]'; 
        }       
...
function coastal_more_link() {
    return '... <a class="more-link" href="' . get_permalink() . '">'.__('Read More', 'coastal').'</a>';
}
...
function cstl2_custom_excerpt_more() {
  ...
  echo '<a href="' . get_permalink() . '" class="more-link custom-more">' . __( 'Read More', 'coastal' ) . '</a>';
    }
  }
}
...
function blog_share_box() { 
        ...
        if ( is_home() ) { 
            ...
            <h3>' , _e('Share') , '</h3>
                ...
                <a class="addthis_button_email">' , __('Email', 'coastal') , '</a>
                </div>
...
function stnsvn_comment_form_defaults( $defaults ) {
    ...
    $defaults['title_reply'] = __( 'Comments', 'coastal' );
    return $defaults;
}
...
function stnsvn_comment_placeholders( $fields )
{
    ...
    $fields['email'] = str_replace(
        ...
        '<input type="email" placeholder="'.__('Email', 'coastal').'"  id="email" name="email"',
        $fields['email']
    );
    $fields['url'] = str_replace(
        '<input id="url" name="url" type="url"',
        '<input placeholder="'.__('Url', 'coastal').'" id="url" name="url" type="url"',
        $fields['url']
    );
    return $fields;
}
     ...
     if( is_page_template( 'portfolio-page.php' ) && !is_front_page()) { ?>
        <div class="back-to-top">
            <a href="<?php echo home_url(); ?>">
                <h4 class="button"><?php _e('Go Back', 'coastal')?></h4>
            </a>
        </div>
<?php }
}
...
function back_to_blog() {
    ...
    <div class="back-to-top">
        <a href="<?php echo get_permalink( get_option( 'page_for_posts' ) ); ?>">
                <h4 class="button"><?php _e('Back to Blog', 'coastal');?></h4>
        </a>
    </div>
<?php }
}
...
function coastal_portfolio() {
  $labels = array(
    'name'               => _x( 'Portfolio', 'post type general name', 'coastal' ),
    'singular_name'      => _x( 'Portfolio', 'post type singular name', 'coastal' ),
    'add_new'            => _x( 'Add New', 'portfolio', 'coastal' ),
    'add_new_item'       => __( 'Add New Portfolio Item', 'coastal' ),
    'edit_item'          => __( 'Edit Portfolio Item', 'coastal' ),
    'new_item'           => __( 'New Portfolio Item', 'coastal' ),
    'all_items'          => __( 'All Portfolio Items', 'coastal' ),
    'view_item'          => __( 'View Portfolio Item', 'coastal' ),
    'search_items'       => __( 'Search Portfolio Items', 'coastal' ),
    'not_found'          => __( 'No Portfolio Items found', 'coastal' ),
    'not_found_in_trash' => __( 'No Portfolio Items found in the Trash', 'coastal' ), 
    'parent_item_colon'  => '',
    'menu_name'          => __( 'Portfolio', 'coastal' )
  );
  $args = array(
    'labels'        => $labels,
    'description'   => __('Holds all portfolio items', 'coastal'),
    'public'        => true,
    'menu_position' => 5,
    'supports'      => array( 'title', 'editor', 'thumbnail', 'excerpt', 'comments' ),
    'has_archive'   => true,
    'rewrite'       => array( 'slug' => _x('portfolio', 'Permalink base for portfolio pages. MUST re-save permalinks in WP admin after editing this.', 'coastal') ),
  );
...
function create_portfolio_category () {
    ...
    $labels = array(
        'name'              => _x( 'Categories', 'taxonomy general name', 'coastal' ),
        'singular_name'     => _x( 'Category', 'taxonomy singular name', 'coastal' ),
        'search_items'      => __( 'Search Categories', 'coastal' ),
        'all_items'         => __( 'All Categories', 'coastal' ),
        'parent_item'       => __( 'Parent Category', 'coastal' ),
        'parent_item_colon' => __( 'Parent Category:', 'coastal' ),
        'edit_item'         => __( 'Edit Category', 'coastal' ),
        'update_item'       => __( 'Update Category', 'coastal' ),
        'add_new_item'      => __( 'Add New Category', 'coastal' ),
        'new_item_name'     => __( 'New Category Name', 'coastal' ),
        'menu_name'         => __( 'Categories', 'coastal' ),
    );
    $args = array(
        'hierarchical'      => true,
        'labels'            => $labels,
        ...
        'query_var'         => true,
        'rewrite'           => array( 'slug' => _x('portfolio-category', 'Permalink base for portfolio pages. MUST re-save permalinks in WP admin after editing this.', 'coastal') ),
    );
...
//-----------------------Landing Header Section -----------------------//
   ...
   $wp_customize->get_section('header_image')->title=__('Landing Header', 'coastal');
   ...
   $wp_customize->get_control('header_image')->priority=20;
        ...
        'header-headline-box',
        array(
            'label'          => __('Headline Text', 'coastal'),
            'section'        => 'header_image',
            'settings'       => 'header-headline-box',
            'type'           => 'text',
            'description'    => __('Use this field to display a boxed headline in the header section', 'coastal'),
                )
            )
        );
        ...
        'header-textbox',
        array(
            'label'          => __('Header Overlay Text', 'coastal'),
            'section'        => 'header_image',
            'settings'       => 'header-textbox',
            'type'           => 'textarea',
            'description'    => __('Use this field to add any additional overlay content to the header area, like a button or a call to action. Accepts html tags like &#12296;h1&#12297;&#12296;strong&#12297;&#12296;img&#12297;&#12296;a&#12297; etc.', 'coastal'),
                )
            )
        );
        ...
        //Homepage Header Text Color
            ...
            new WP_Customize_Color_Control(
                ...
                'header_txt_color',
                array(
                    'label' => __('Homepage Header Text Color', 'coastal'),
                    'section' => 'header_image',
        ...
        $wp_customize->add_control(
            ...
            array(
                'type' => 'checkbox',
                'label' => __('Fix header image on scroll', 'coastal'),
                'section' => 'header_image',
            )
        );
...
//-----------------------Theme Style Section -----------------------//        
  // Add Theme Styles section  
    $wp_customize->add_section(
        'theme-style',
        array(
            'title' => __('Theme Colors', 'coastal'),
            'description' => __('Note: home page section colors are controlled individually via the page editor.', 'coastal'),
            'priority' => 21,
        )
    );
        ...
        //Border color
            ...
            new WP_Customize_Color_Control(
                ...
                'coastal-border-color',
                array(
                    'label' => __('Element Border Color', 'coastal'),
                    'section' => 'theme-style',
        ...
        //Footer background color
            ...
            new WP_Customize_Color_Control(
                ...
                'footer-background-color',
                array(
                    'label' => __('Footer Background Color', 'coastal'),
                    'section' => 'theme-style',
        ...
        //Primary background color
            ...
            new WP_Customize_Color_Control(
                ...
                'primary-background-color',
                array(
                    'label' => __('Primary Background Color', 'coastal'),
                    'section' => 'theme-style',
        ...
        //Secondary background color
            ...
            new WP_Customize_Color_Control(
                ...
                'secondary-background-color',
                array(
                    'label' => __('Secondary Background Color', 'coastal'),
                    'section' => 'theme-style',
        ...
        //Button hover color
            ...
            new WP_Customize_Color_Control(
                ...
                'btn-hover-color',
                array(
                    'label' => __('Button Hover Color', 'coastal'),
                    'section' => 'theme-style',
...
//-----------------------Typography Section -----------------------//
      // Add Typography section  
        $wp_customize->add_section(
            'typography',
            array(
                'title' => __('Typography', 'coastal'),
                'description' => __('Customize the site typography here. Insert Google Fonts code (eg. http://fonts.googleapis.com/css?family=Lora) and then enter the font name (eg. Lora) in the desired section.', 'coastal'),
                'priority' => 22,
            )
        );
      //Typography Settings
      //Google Font Link Field
        ...
        $wp_customize->add_control(
            ...
            array(
                'type' => 'text',
                'label' => __('Google Font Link Code', 'coastal'),
                'section' => 'typography',
            )
        );
        //Primary Font
        ...
        $wp_customize->add_control(
            'primary_font_family',
            array(
                'label' => __('Primary Font Family', 'coastal'),
                'section' => 'typography',
            )
        );
        //Secondary Font
        ...
        $wp_customize->add_control(
            'secondary_font_family',
            array(
                'label' => __('Body Text Font Family', 'coastal'),
                'section' => 'typography',
            )
        );
         //Primary text color
            ...
            new WP_Customize_Color_Control(
                ...
                array(
                    ...
                    'description' => __('Note: home page section text colors are controlled individually via the page editor.', 'coastal'),
                )
            )
        );
        //Link color
...
//-----------------------Portfolio Pages Section -----------------------//
    $wp_customize->add_section(
            'portfolio_pages',
            array(
                'title' => __('Portfolio Pages', 'coastal'),
                'description' => __('These settings apply for Portfolio pages only.', 'coastal'),
                'priority' => 24,
            )
        );
        ...
        $wp_customize->add_control(
            ...
            array(
                'type' => 'select',
                'label' => __('Select portfolio sidebar style:', 'coastal'),
                'section' => 'portfolio_pages',
                'choices' => array(
                    'content-sidebar' => __('Content, Primary Sidebar', 'coastal'),
                    'full-width-content' => __('Full Width Content', 'coastal'),
                ),
            )
        );
        ...
        $wp_customize->add_control(
            new WP_Customize_Control(
                ...
                'backbtn-url',
                array(
                    'label' => __('Back Button URL', 'coastal'),
                    'section' => 'portfolio_pages',
        ...
        $wp_customize->add_control(
            new WP_Customize_Control(
                ...
                'backbtn-text',
                array(
                    'label' => __('Back Button Text', 'coastal'),
                    'section' => 'portfolio_pages',
        ...
        $wp_customize->add_control(
            ...
            array(
                'type' => 'checkbox',
                'label' => __('Display share buttons', 'coastal'),
                'section' => 'portfolio_pages',
            )
        );
...
//----------------------- Blog Index Section -----------------------//
        ...
        $wp_customize->add_section(
            'blog_post',
            array(
                'title' => __('Blog Index', 'coastal'),
                'description' => __('These settings apply to main blog index or blog archive page only.', 'coastal'),
                'priority' => 25,
            )
        );
        ...
        $wp_customize->add_control(
            'blog_layout',
            array(
                'label'    => __( 'Select Default Layout', 'coastal' ),
                'section'  => 'blog_post',
        ...
        $wp_customize->add_control(
            ...
            array(
                'type' => 'checkbox',
                'label' => __('Display post date', 'coastal'),
                'section' => 'blog_post',
            )
        );
        ...
        $wp_customize->add_control(
            ...
            array(
                'type' => 'checkbox',
                'label' => __('Display author byline', 'coastal'),
                'section' => 'blog_post',
            )
        );
        ...
        $wp_customize->add_control(
            ...
            array(
                'type' => 'checkbox',
                'label' => __('Display comment count', 'coastal'),
                'section' => 'blog_post',
            )
        );
        ...
        $wp_customize->add_control(
            ...
            array(
                'type' => 'checkbox',
                'label' => __('Display share links on main blog page', 'coastal'),
                'section' => 'blog_post',
            )
        );
        ...
        $wp_customize->add_control(
            'coastal_addthis_pub_id',
            array(
                'label' => __('AddThis Publisher ID', 'coastal'),
                'section' => 'blog_post',
                'description' => __('Paste your AddThis publisher ID here to get analytics on how your content is shared.', 'coastal'),
            )
        );
...
//----------------------- Single Blog Posts Section -----------------------//
        ...
        $wp_customize->add_section(
            'single_blog_post',
            array(
                'title' => __('Single Blog Posts', 'coastal'),
                'description' => __('These settings apply to individual blog posts only.', 'coastal'),
                'priority' => 26,
            )
        );
        ...
        $wp_customize->add_control(
            ...
            array(
                'type' => 'checkbox',
                'label' => __('Display featured image on single posts', 'coastal'),
        ...
        $wp_customize->add_control(
            ...
            array(
                'type' => 'checkbox',
                'label' => __('Display categories', 'coastal'),
                'section' => 'single_blog_post',
            )
        );
        ...
        $wp_customize->add_control(
            ...
            array(
                'type' => 'checkbox',
                'label' => __('Display share links on single posts', 'coastal'),
                'section' => 'single_blog_post',
            )
        );
        ...
        $wp_customize->add_control(
            ...
            array(
                'type' => 'checkbox',
                'label' => __('Display "Go Back" button on posts', 'coastal'),
                'section' => 'single_blog_post',
            )
        );
        ...
        $wp_customize->add_control(
            ...
            array(
                'type' => 'select',
                'label' => __('Choose number of columns for footer', 'coastal'),
                'section' => 'footer_settings',
                'choices' => array(
                    'footer-col-1' => __('1 column', 'coastal'),
                    'footer-col-2' => __('2 columns', 'coastal'),
                    'footer-col-3' => __('3 columns', 'coastal'),
                    'footer-col-varied-3' => __('3 columns varied width', 'coastal'),
                    'footer-col-4' => __('4 columns', 'coastal'),
                ),
            )
        );
       ...
       //Copyright footer text section
        ...
        $wp_customize->add_control(
            new WP_Customize_Control(
                ...
                'copyright-footer',
                array(
                    'label' => __('Footer Copyright Text', 'coastal'),
                    'section' => 'footer_settings',
                    'settings' => 'copyright-footer',
                    'type' => 'text',
                    'description' => __('Add text to display in the footer copyright section.', 'coastal')
                )
            )
        );
        ...
        $wp_customize->add_control(
            new WP_Customize_Control(
                ...
                'stnsvn-credit',
                array(
                    'label' => __('Hide Station Seven credit?', 'coastal'),
                    'section' => 'footer_settings',
                    'settings' => 'stnsvn-credit',
                    'type' => 'checkbox',
                    'description' => __('While of course not required, we appreciate any support as we grow our small business :)', 'coastal')
                )
            )
        );
...
//-----------------------Navigation Menu -----------------------//
      // Add Navigation section  
        $wp_customize->add_section(
            'navigation',
            array(
                'title' => __('Navigation Style', 'coastal'),
                'description' => __('Customize the look of the nav menus here.', 'coastal'),
                'priority' => 100,
            )
        );
        ...
        //Secondary nav menu background color
            ...
            new WP_Customize_Color_Control(
                ...
                'sec_nav_bg_color',
                array(
                    'label' => __('Secondary Nav Background Color', 'coastal'),
                    'section' => 'navigation',
        ...
        //Secondary nav menu text color
            ...
            new WP_Customize_Color_Control(
                ...
                'sec_nav_text_color',
                array(
                    'label' => __('Secondary Nav Text Color', 'coastal'),
                    'section' => 'navigation',
        ...
        $wp_customize->add_control(
            ...
            array(
                'type' => 'checkbox',
                'label' => __('Disable secondary nav stickiness', 'coastal'),
                'section' => 'navigation',
            )
        );
        ...
        $wp_customize->add_control(
            ...
            array(
                'type' => 'checkbox',
                'label' => __('Disable secondary nav transparency', 'coastal'),
                'section' => 'navigation',
            )
        );
...
//-----------------------Custom CSS Section -----------------------//
      // Add CSS section  
        $wp_customize->add_section(
            'coastal_css',
            array(
                'title' => __('Custom CSS', 'coastal'),
                'description' => __('Add any custom CSS here.', 'coastal'),
                'priority' => 100,
            )
        );
        $wp_customize->add_setting( 
    ...
    //-----------------------Development Section -----------------------//
      // Add Development section 
        ...
        $wp_customize->add_control(
            ...
            array(
                'type' => 'checkbox',
                'label' => __('Display ACF Pro admin menu', 'coastal'),
                'description' => __('The Advanced Custom Fields Pro plugin is included free with the Coastal theme. Check this box to enable and access the configuration area in your WP dashboard.', 'coastal'),
                'section' => 'coastal_development',
            )
        );
